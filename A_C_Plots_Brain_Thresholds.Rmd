...
theme_demographics <- theme_bw() +
  theme(
    ...
    theme.background = element_rect(fill=NA)
  )

```

```{r}
Regions_Activated <- read_csv("M:/Documents/Hariri_review/USE_ME/Activations/Controls/Spreadsheets/Regions_activated_CONTROLS_NEW.csv", show_col_types = FALSE)
...
```{r}
REGIONS <- (setdiff(ls(), c("Total_counts", "REGIONS", "Regions_Activated", "Regions_Activated_L", "Regions_Activated_R", "df", "df_A", "df_S", "cols_to_plot_searched", "cols_to_plot_activated", "r", "MaxVals", "ROIfill")))
REGIONS <- Regions_Activated$Region
Regions_Activated$Th5_Applied <- 26
Regions_Activated$Th6_Applied <- 8
Regions_Activated$Th7_Applied <- 41
Regions_Activated$Th8_Applied <- 18
#Regions_Activated$Th9_Applied <- 5

#cols_to_plot_searched <- c("Th1_Searched", "Th2_Searched", "Th3_Searched", "Th4_Searched")
#cols_to_plot_activated <- c("Th1_Activated","Th2_Activated", "Th3_Activated", "Th4_Activated","Th5_Activated","Th6_Activated", "Th7_Activated", "Th8_Activated", "Th9_Activated")
#cols_to_plot_applied <-  c("Th5_Applied","Th6_Applied", "Th7_Applied", "Th8_Applied", "Th9_Applied")

REGIONS = c("Amygdala", "Hippocampus", "Cingulate", "Temporal_Superior","Parietal_Superior","Occipital_V1")
...
#Temporal - clockwise
  ...
  if (r == "Temporal_Pole"){ROIfill <- c("#97d5bf", "#E0F2EC", "white")}
...
#Occipital - clockwise
  ...
  ggsave(plot = PLOT, width = 6, height = 6, bg = "transparent", units = "in", filename = paste("M:/Documents/Hariri_review/USE_ME/Activations/Controls/Plots/", r, "_new_X_LARGEFONT.png"), device = cairo_ps())
